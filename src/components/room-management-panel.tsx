                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {TEAM_COUNT_OPTIONS.map((count) => (
                    <button
                      ...
                      className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${draftSettings?.teamCount === count
                ...
                <div className="mt-3 grid grid-cols-4 gap-2">
                  {LOSS_CARD_OPTIONS.map((count) => (
                    <button
                      ...
                      disabled={isBusy || !player.isHost}
                      className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${draftSettings?.lossCardCount === count
                ...
                <div className="mt-3 grid grid-cols-4 gap-2">
                  {([0, 1, 2, 3] as const).map((count) => (
                    <button
                      ...
                      disabled={isBusy || !player.isHost}
                      className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${(draftSettings?.extraRows ?? 0) === count
                          ? "bg-[#2563EB] text-[#F8FAFC]"
                          : "border border-white/15 bg-[#0F172A] text-[#F8FAFC]/85 hover:bg-[#111d34]"
                        } disabled:cursor-not-allowed disabled:opacity-60`}
                    >
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {(["Normal", "Medium", "Hard"] as const).map((level) => (
                    <button
                      ...
                      disabled={isBusy || !player.isHost}
                      className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${(draftSettings?.difficulty ?? "Normal") === level
                          ? level === "Hard"
                            ? "bg-[#DC2626] text-[#F8FAFC]"
                            : level === "Medium"
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {ROLE_OPTIONS.map((role) => (
                    <button
                      ...
                      disabled={setupControlsDisabled}
                      className={`rounded-2xl px-4 py-2.5 text-sm font-bold transition ${player.role === role
                          ? "bg-[#2563EB] text-[#F8FAFC]"
          ...
          <div className="rounded-[2rem] border border-white/10 bg-[#1E293B] p-6 shadow-lg">
            ...
            <div className="mt-6 grid gap-3">
              {room.players.map((entry) => (
                <div
                  key={entry.id}
                  className={`rounded-2xl border p-3 ${isActiveTeam(entry.team)
