...
export function RoomManagementPanel({ mode = "lobby", onClose }: RoomManagementPanelProps) {
  const { room, player, roomId, isBusy, chooseTeam, chooseRole, leaveRoom, startGame, updateRoomSettings, resetGame } =
    useGameRoom();
  ...
  const isPlayingState = room.gameState === "Playing";
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      {isModal ? (
        <div className="sticky top-0 z-20 flex items-center justify-between gap-3 rounded-3xl border border-white/10 bg-[#1E293B]/95 px-5 py-4 shadow-2xl backdrop-blur-sm">
          <div>
            <h2 className="text-2xl font-black text-[#F8FAFC]">إعدادات الغرفة</h2>
            <p className="mt-1 text-sm text-[#F8FAFC]/65">لوحة مشابهة للّوبي لعرض الغرفة وتعديلها.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-2xl border border-white/15 px-4 py-2 text-sm font-bold text-[#F8FAFC] transition hover:bg-white/10"
          >
            إغلاق
          </button>
        </div>
      ...
      {isModal && isPlayingState ? (
        <div className="rounded-[2rem] border border-[#F59E0B]/35 bg-[#1E293B]/90 p-5 text-sm leading-7 text-[#F8FAFC]/82 shadow-lg">
          أي تعديل على إعدادات الغرفة أثناء الجولة سيعيد الجميع إلى اللوبي حتى يتم تطبيق الإعدادات الجديدة بشكل
          صحيح.
          {player.isHost ? (
            <button
              type="button"
              ...
              disabled={isBusy}
              className="mt-4 block rounded-2xl border border-white/15 bg-[#0F172A] px-4 py-2 text-sm font-bold text-[#F8FAFC] transition hover:bg-[#17233a] disabled:cursor-not-allowed disabled:opacity-60"
            >
              إيقاف الجولة والعودة إلى اللوبي
            </button>
          ) : null}
        </div>
      ) : null}
